...
前端为 **Vue 3 + Vite**（`frontend/`），与 FastAPI **前后端分离**开发；生产构建产物在 `frontend/dist/`，若该目录存在则仍可由同一端口 **8000** 托管静态资源。

**开发（推荐：双终端）**

```bash
# 终端 A：仅 API（推荐其一）
# 方式 1：在 backend 目录下务必使用「python + 脚本」，不要单独写 uv run app.py（部分 uv 版本不会执行 __main__）
cd backend
uv run python app.py

# 方式 2：在项目根用 uvicorn（需已存在 backend/__init__.py）
cd ..
uv run uvicorn backend.app:app --host 0.0.0.0 --port 8000 --reload

# 终端 B：前端开发服务器（代理 /auth、/sessions、/chat、/documents 到 8000）
cd frontend
npm install
npm run dev
```

浏览器访问：**http://127.0.0.1:5173/**（页面）+ 接口经 Vite 转发到后端。
...
- 前端页面：`http://127.0.0.1:8000/`（需已执行 `npm run build` 生成 `frontend/dist`）
- API 文档：`http://127.0.0.1:8000/docs`

开发环境下若希望 Vite 代理到其他后端地址，可设置环境变量 **`VITE_DEV_API_PROXY`**（默认 `http://127.0.0.1:8000`）。
...
- 后端：`backend/`
  ...
  - [models.py](backend/sqlbase.py)：ORM 模型定义（用户、会话、消息、父文档）。
  - [cache.py](backend/cache.py)：Redis JSON 缓存封装。
  - [agent.py](backend/crud.py)：LangChain Agent、会话存储、摘要逻辑。
