...
catRAG（喵呜助手）是一个面向法律知识场景的 RAG（检索增强生成）智能问答系统。系统基于外部法律知识库，通过向量检索精准召回相关文档，再交由大模型生成回答——相比直接调用大模型，大幅降低 token 消耗，同时保证回答可溯源、有依据。

**核心流程：** 用户提问 → BGE-M3 向量检索知识库 → LangGraph 编排评分与路由 → 大模型基于检索结果生成回答

**为什么用 RAG？**
- Embedding 向量化（BGE-M3）免费，向量检索无 token 消耗
- 仅评分、精排、生成阶段调用大模型，token 用量少，成本低
- 知识库可随时更新，无需重新训练模型
- 回答基于真实文档，可追溯来源，减少幻觉
